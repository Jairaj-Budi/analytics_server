import { Controller, Get, HttpException, HttpStatus, Param, ParseIntPipe, Post, Req, Res, UseGuards, UseInterceptors } from '@nestjs/common';
...
export class NestFeaturesController {
    ...
    sampleException() {
        try {
            ...
            throw new HttpException({ status: HttpStatus.AMBIGUOUS, error: 'I am testing the ambigious exception' }, HttpStatus.AMBIGUOUS, { cause: err })
        }
    }
    @Get('customValidationErrors/:id')
    setCustomValidationError(@Param('id', new ParseIntPipe({ errorHttpStatusCode: HttpStatus.NOT_ACCEPTABLE })) id: number) {
        try {
            console.log(id, 'id')
